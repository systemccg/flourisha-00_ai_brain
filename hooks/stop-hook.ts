...
// Voice features disabled for remote server deployment
// Future options:
// 1. Stream audio over SSH (pulseaudio forwarding)
// 2. WebSocket audio in Flourisha web app
// 3. Local voice server with remote trigger API
...
async function main() {
  ...
  // Voice notification (disabled for remote server)
  // To enable: set VOICE_ENABLED = true and ensure voice server is running
  if (VOICE_ENABLED && message) {
    // Future: implement voice notification via web app or SSH audio forwarding
    console.error(`🔊 Voice would say: "${message}"`);
  }
  ...
  // Set tab title (uses escape sequences, no shell spawn needed)
  if (tabTitle) {
    try {
      // Direct escape sequence output - no execSync/shell needed
