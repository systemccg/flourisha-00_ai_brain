 ...
 *
 * Automatically loads your Flourisha skill context at session start by reading and injecting
 * the PAI SKILL.md file contents directly into Claude's context as a system-reminder.
 *
 * Adapted from Daniel Miessler's PAI (2025-11-20) with centralized path resolution
 *
 * Purpose:
...
async function main() {
  try {
    ...
    if (isSubagent) {
      // Subagent sessions don't need Flourisha context loading
      console.error('🤖 Subagent session - skipping Flourisha context loading');
      process.exit(0);
    }

    // Get Flourisha skill path using centralized path resolution
    const paiSkillPath = join(SKILLS_DIR, 'CORE/SKILL.md');

    // Verify Flourisha skill file exists
    if (!existsSync(paiSkillPath)) {
      console.error(`❌ Flourisha skill not found at: ${paiSkillPath}`);
      console.error(`💡 Create your Flourisha skill file or check PAI_DIR environment variable`);
      process.exit(1);
    }

    console.error('📚 Reading Flourisha core context from skill file...');

    // Read the Flourisha SKILL.md file content
    const paiContent = readFileSync(paiSkillPath, 'utf-8');

    console.error(`✅ Read ${paiContent.length} characters from Flourisha SKILL.md`);

    // Output the Flourisha content as a system-reminder
...
This context is now active for this session. Follow all instructions, preferences, and guidelines contained above.
    ...
    console.error('✅ Flourisha context injected into session');
    process.exit(0);
  } catch (error) {
