 ...
 * Updates tab title based on user prompt and caches it for statusline
 *
...
async function main() {
  try {
    ...
    // Cache the last prompt for statusline display (per-session)
    try {
      const sessionId = data.session_id || 'default';
      const cacheFile = `/tmp/.claude_last_prompt_${sessionId}`;
      const cleanPrompt = prompt.replace(/\n/g, ' ').trim();
      writeFileSync(cacheFile, cleanPrompt, 'utf-8');
    } catch (e) {
      // Silently fail
    }
