 ...
 * This hook provides comprehensive event tracking for the Flourisha observability system
 *
...
function getAgentForSession(sessionId: string): string {
  try {
    ...
    if (existsSync(mappingFile)) {
      ...
      // Default agent name for Flourisha
      return mappings[sessionId] || 'flourisha';
    }
  } catch (error) {
    // Ignore errors, default to primary agent
  }
  // Default to Flourisha
  return 'flourisha';
}
...
async function main() {
  try {
    ...
    // If this is a SubagentStop or Stop event, reset to Flourisha
    else if (eventType === 'SubagentStop' || eventType === 'Stop') {
      agentName = 'flourisha';
      setAgentForSession(sessionId, 'flourisha');
    }
