/**
 * Flourisha Path Resolution - Single Source of Truth
 *
 * This module provides consistent path resolution across all Flourisha hooks.
...
/**
 * Flourisha-specific directories
 */
export const FLOURISHA_DIR = resolve(homedir(), 'flourisha');
export const AI_BRAIN_DIR = join(FLOURISHA_DIR, '00_AI_Brain');
...
function validatePAIStructure(): void {
  if (!existsSync(PAI_DIR)) {
    console.error(`Flourisha PAI_DIR does not exist: ${PAI_DIR}`);
    console.error(`   Expected ~/.claude or set PAI_DIR environment variable`);
    process.exit(1);
  }

  if (!existsSync(HOOKS_DIR)) {
    console.error(`Flourisha hooks directory not found: ${HOOKS_DIR}`);
...
/**
 * Get the Flourisha Google Drive sync directory
 */
export function getFlourishaDir(): string {
  return FLOURISHA_DIR;
}

/**
 * Get AI Brain skills directory (symlinked from ~/.claude/skills)
 */
export function getAIBrainSkillsDir(): string {
  return join(AI_BRAIN_DIR, 'skills');
}
