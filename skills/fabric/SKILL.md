...
**Pattern Location:** `~/flourisha/00_AI_Brain/skills/fabric/fabric-repo/data/patterns/`

**To read a pattern:**
```bash
cat ~/flourisha/00_AI_Brain/skills/fabric/fabric-repo/data/patterns/[pattern_name]/system.md
```

**If Fabric CLI is NOT installed:** Read the pattern's `system.md` file and apply it manually to the content. The pattern file contains the exact instructions.
...
cat file.txt | fabric -p analyze_claims
```
...
if [ ! -d "$HOME/flourisha/00_AI_Brain/skills/fabric/fabric-repo" ]; then
  echo "Fabric repository not found. Cloning..."
  cd "$HOME/flourisha/00_AI_Brain/skills/fabric"
  git clone https://github.com/danielmiessler/fabric.git fabric-repo
  echo "Fabric repository cloned successfully."
else
  echo "Fabric repository found at $HOME/flourisha/00_AI_Brain/skills/fabric/fabric-repo"
fi
```
