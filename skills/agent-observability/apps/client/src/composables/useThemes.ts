...
export function useThemes() {
  ...
  const getApiHost = () => window.location.hostname === 'localhost' ? 'localhost' : window.location.hostname;

  const saveThemeToServer = async (theme: CustomTheme): Promise<void> => {
    const apiHost = getApiHost();
    const response = await fetch(`http://${apiHost}:4000/api/themes`, {
      method: 'POST',
  ...
  const loadThemesFromServer = async (): Promise<CustomTheme[]> => {
    try {
      const apiHost = getApiHost();
      const response = await fetch(`http://${apiHost}:4000/api/themes?isPublic=true`);
